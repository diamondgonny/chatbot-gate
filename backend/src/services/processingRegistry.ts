...
class ProcessingRegistry {
  ...
  /**
   * Close all connected clients for a processing entry
   */
  private closeClients(processing: ActiveProcessing): void {
  ...
  abort(userId: string, sessionId: string): void {
    ...
    if (processing) {
      processing.abortController.abort();
      console.log(`[ProcessingRegistry] Aborted processing for ${key}`);
    }
